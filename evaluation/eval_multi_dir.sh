...
MODEL_EVAL_DIR="/home/yifan50/rl/deepscaler/output/models/final_teacher"

MODEL_DIRS=()

for d in "$MODEL_EVAL_DIR"/*; do
    if [[ -d "$d" ]]; then
        MODEL_DIRS+=("$d")
    fi
done

echo "Discovered ${#MODEL_DIRS[@]} model directories under $MODEL_EVAL_DIR:"
...
for DATA_NAME in "${DATA_NAMES[@]}"; do
    for RAW_MODEL_DIR in "${MODEL_DIRS[@]}"; do
        ...
        if [[ "$DATA_NAME" == "aime24" || "$DATA_NAME" == "amc23" ]]; then
            N_SAMPLING=8
        else
            N_SAMPLING=1
        fi
